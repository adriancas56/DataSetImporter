        ...
        <table class="w-full tableClass">
            ...
            <tbody>
                <tr v-for="categoryItem in categories" :key="categoryItem._id" :category="categoryItem">
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm " @click="redirectToCategoryDetails(categoryItem.name)">
                    ...
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm" @click="redirectToCategoryDetails(categoryItem.name)">
                          <p class="text-gray-900 whitespace-no-wrap">
                            {{categoryItem.userUpdate}}
                        </p>
                    </td>
                    <td class="flex space-x-4 justify-end py-2 pr-3 border-b border-gray-200 bg-white text-sm ">
                        <button class="hover:bg-white p-3 rounded-full" @click="emit('delete-category', categoryItem._id)">
                            <svg class="text-gray-900 " style="width:24px;height:24px" viewBox="0 0 24 24">
                                <path fill="currentColor" d="M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19Z" />
                            </svg>
                        </button>
                        <button class="hover:bg-white p-3 rounded-full" @click="redirectToCategoryDetails(categoryItem.name)">
